...
var $author$project$Data$Video$Gif$Gif = F4(
	function (localObjectKey, fileSize, width, fps) {
		return {fileSize: fileSize, fps: fps, localObjectKey: localObjectKey, width: width};
	});
...
var $author$project$Data$Video$Gif$decoder = A5(
	...
	A2($elm$json$Json$Decode$field, 'fps', $elm$json$Json$Decode$float));
...
var $author$project$Component$Project$Main$initLoaded = function (project) {
	return A2(
		...
		$author$project$Data$UndoList$fresh(project),
		{
			...
			poster: $elm$core$Maybe$Nothing,
			projectExported: $elm$core$Maybe$Nothing,
...
var $author$project$Component$Project$Msg$ExportProject = function (a) {
	return {$: 'ExportProject', a: a};
};
...
var $author$project$Port$copyAll = _Platform_outgoingPort('copyAll', $elm$json$Json$Encode$string);
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				'width',
				$elm$json$Json$Encode$int(gif.width)),
				_Utils_Tuple2(
				'fps',
				$elm$json$Json$Encode$float(gif.fps))
			]));
};
	...
	function (newFps, gif) {
		return _Utils_update(
			gif,
			{
				fps: A2(
					$elm$core$Maybe$withDefault,
					$author$project$Data$Env$defaultGifFps,
					$elm$core$String$toFloat(newFps))
			});
	});
var $author$project$Data$Project$Content$changeGifFps = F3(
	function (gif, newFps, content) {
		if (content.$ === 'FromSegment') {
			var isContentHidden = content.a;
			var segmentContent = content.b;
			return A2(
				...
				isContentHidden,
				A2(
					$author$project$Data$Project$SegmentContent$addGif,
					A2($author$project$Data$Video$Gif$changeGifFps, newFps, gif),
					segmentContent));
		} else {
			return content;
		}
	...
	function (newWidth, gif) {
		return _Utils_update(
			gif,
			{
				width: A2(
					$elm$core$Maybe$withDefault,
					$author$project$Data$Env$defaultGifWidth,
					$elm$core$String$toInt(newWidth))
			});
	});
var $author$project$Data$Project$Content$changeGifWidth = F3(
	function (gif, newWidth, content) {
		if (content.$ === 'FromSegment') {
			var isContentHidden = content.a;
			var segmentContent = content.b;
			return A2(
				...
				A2(
					$author$project$Data$Project$SegmentContent$addGif,
					A2($author$project$Data$Video$Gif$changeGifWidth, newWidth, gif),
					segmentContent));
		} else {
			return content;
		}
					...
					_Utils_update(
						present,
						{
							workingData: A3(
								$elm_community$array_extra$Array$Extra$update,
								index,
								A2($author$project$Data$Project$Content$changeGifWidth, gif, newWidth),
								present.workingData)
						}));
			case 'ChangeFps':
				var gif = msg.a;
					...
					_Utils_update(
						present,
						{
							workingData: A3(
								$elm_community$array_extra$Array$Extra$update,
								index,
								A2($author$project$Data$Project$Content$changeGifFps, gif, newFps),
								present.workingData)
						}));
			default:
				var index = msg.a;
		...
		switch (msg.$) {
			...
			case 'StopPreviewing':
				return _Utils_Tuple3(
					...
					_Utils_update(
						substate,
						{isCopied: false, isPreviewing: false, projectExported: $elm$core$Maybe$Nothing}),
					$elm$core$Platform$Cmd$none);
			case 'CopyAll':
				var prompt = msg.a;
				return _Utils_Tuple3(
					...
					_Utils_update(
						substate,
						{isCopied: true}),
					$author$project$Port$copyAll(prompt));
			case 'SaveProject':
			...
			case 'SaveAndExportProject':
				return _Utils_Tuple3(
					project,
					_Utils_update(
						substate,
						{savingResult: $elm$core$Maybe$Nothing}),
					A2($author$project$Request$Project$save, $author$project$Component$Project$Msg$ExportProject, present));
				...
				if (msg.a.$ === 'Ok') {
					...
					return _Utils_Tuple3(
						...
						_Utils_update(
							...
							{
								isExportingInProgress: false,
								projectExported: $elm$core$Maybe$Just(
									_Utils_Tuple2(base, info))
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var error = msg.a.a;
			...
			case 'MsgForEditing':
				...
				var _v4 = A4($author$project$Component$Project$Editing$update, key, editingMsg, project, substate);
				var proj = _v4.a;
				var subs = _v4.b;
				var cmd = _v4.c;
				var pcmd = A2(
...
var $author$project$Component$Project$Msg$CopyAll = function (a) {
	return {$: 'CopyAll', a: a};
};
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							_Utils_Tuple2('absolute inset-0 bg-black flex flex-row justify-start items-center', true),
							_Utils_Tuple2('invisible', !displayModal)
						]))
				]),
			_List_fromArray(
				[
...
var $author$project$Translations$Page$Project$LeftPanel$copyAll = function (translations) {
	return A2($ChristophP$elm_i18next$I18Next$t, translations, 'page.project.left_panel.copy_all');
};
var $author$project$Translations$Page$Project$LeftPanel$copyAllPrompt = function (translations) {
	return A2($ChristophP$elm_i18next$I18Next$t, translations, 'page.project.left_panel.copy_all_prompt');
};
...
var $author$project$Component$Project$View$Preview$viewPreviewer = F2(
	function (_v0, base) {
		...
		return A2(
			...
			_List_fromArray(
				[
					$elm$html$Html$Attributes$id('copyAllTarget'),
...
var $author$project$Component$Project$View$Preview$view = F9(
	function (trn, noOp, stopPreviewing, copyAll, projectSize, projectExported, isPreviewing, isCopied, project) {
		var viewFrom = F2(
			function (base, info) {
				return A4(
					$author$project$View$Layout$viewOverlay,
...
var $author$project$Component$Project$Msg$ChangeFps = F3(
	function (a, b, c) {
		return {$: 'ChangeFps', a: a, b: b, c: c};
	});
var $author$project$Component$Project$Msg$ChangeWidth = F3(
	function (a, b, c) {
		return {$: 'ChangeWidth', a: a, b: b, c: c};
	});
...
var $author$project$Data$Video$Gif$getFps = function (_v0) {
	var fps = _v0.fps;
	return fps;
};
...
var $author$project$Data$Video$Gif$toFpsStr = function (_v0) {
	var fps = _v0.fps;
	return $elm$core$String$fromFloat(fps);
};
...
var $author$project$Component$Project$Msg$SaveAndExportProject = {$: 'SaveAndExportProject'};
				...
				A2(
					...
					_List_fromArray(
						[
							...
							$author$project$Component$Project$Msg$SaveAndExportProject),
							_Utils_Tuple3(
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A9($author$project$Component$Project$View$Preview$view, trn, $author$project$Component$Project$Msg$NoOp, $author$project$Component$Project$Msg$StopPreviewing, $author$project$Component$Project$Msg$CopyAll, substate.projectSize, substate.projectExported, substate.isPreviewing, substate.isCopied, project),
					A6(
